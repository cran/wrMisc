...
The aim of `unifyEnumerator()` is to provide help in automatically harmonizing enumerators at the end of sample-names.
...
This function will work only, if the numerator, ie digit-character(s) are ate the end of the character-string.
...
### Order Matrix Accordinf To Reference
...
For this reason the argument _invertCount_ was introduced, in this case a line wit a 0 accurring characterizes a constant behaviour (for the respective column).
...
For example this may be the case when comparing with tehcnical replicates only.
