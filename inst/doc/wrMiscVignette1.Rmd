...
Most of additional packages used here at some point were declared as 'suggested' (ie not obligatory), to allow installation of _wrMisc_ even if some these additional packages can't be installed/compiled by the user's instance.
...
(so called 'Sandwich' nodes as they are surouded by initial nodes), for such nodes a separate (eg even more stringent) threshold can be applied.
