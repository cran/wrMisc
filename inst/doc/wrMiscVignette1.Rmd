...
This package contains a collection of various (low-level) tools which may be of general interest, in particular in the context of working with 'omics'-data. 
...
Of course, numerous other documents/videos/sites with tutorials and courses exist, too.
...
Most of additional packages used in some of the functions were declared as 'suggested' (ie not obligatory), to allow installation of _wrMisc_ even if some these additional packages can't be installed/compiled at the user's side.
...
Let's assume we have measures for 200 features (eg gene-abundaces) from 8 groups of samples :
...
It is also possible to return the ratio of NA values or both the number of _NAs_ and the size of the sub-groups.
```{r rowGrpNA3, echo=TRUE}
head(rowGrpNA(mat2, grp2, mode="ratio"))
head(rowGrpNA(mat2, grp2, mode="complete"))
```

This function allows also treating objects created by testing from package [wrProteo](https://CRAN.R-project.org/package=wrProteo), the group-information is typically already available in such objects.
Finally, all results may get collapsed in single column/vector when using any of the _..Collapse_ modes.
```{r rowGrpNA4, echo=TRUE}
...
In order to combine list-elements into a data.frame (or matrix), base R can do the job - as long as there is no problem with the number of rows.
...
### Comparing Global Patterns        {#ComparingGlobalPatterns}
...
Please see also the function _chooseGroupNames()_ which may help choosing among different types of patterns (ie low or high number of groups of replicates) (see section [Working With Redundant Data] (#WorkingWithRedundantData)).
...
### Trimming Text Character To Specific Length

The function `truncateVect()` gives for convenience more options than _substr_.
It is possible to add extra text when truncation has occured (default '...') so that the user can see that truncation of longer entries has been performed.

Furthermore, using the argument _sep it is possible to respect entities between separators (within a string of characters) and only cut at/before such separators.
The resulting text vecor may thus be shorter than specified globally with argument _nMax_.

```{r truncateVect1, echo=TRUE}
...
A more gentle way to make text non-redundant consists in removing redundant text isporposed by the function `rmSharedWords()`.
Multiple separators may be used (and combined) to specify 'words', a minimum length of what mat be considered a word can be defined, too (default at min 2 characters).
For example, this can be used to make column-names shorter and more precise.

```{r rmSharedWords1, echo=TRUE}
...
When information about a collection of samples is given in a table, the function  `unifyEnumerator()` was designed to find the column most likely containg sample-names 
and it derives then the structure of groups of replicates by removing terminal enumerators from sample-names.
Initial tables (as frequently found in transcriptomics and proteomics) may not have consistent schemes of naming columns (due to human annotators not using controlled vocabulary) 
and may contain additional columns with redundant information (elements/conditions that were kept constant for all samples.    

Internally this function uses _rmSharedWords()_ to further simplify resultant names.

```{r chooseGroupNames1, echo=TRUE}
...
Please see also the function _replicateStructure()_ which allows inspecting patterns of replicating elements (see section [Comparing Global Patterns] (#ComparingGlobalPatterns)).
...
The function `mergeVectors()` allows merging of multiple named vectors (each element needs to be named !).
...
In the example below we'll add another vector, nuty this time _without_ named elements.
In the absence of names there are no anchors and in consequence the last vector cannot get merged to the rest.
You can see a message telling that this vector was ignored/omitted.
...
The function `orderMatrToRef()` has the aim of facilitating bringing a matrix of text/data in the order of a given reference (character vector).
...
### Reading/Importing Incomplete Tables
...
GitHub (https://github.com) allows sharing code and data.
In order to properly read tabulated (txt, tsv or csv) data directly from a given url from the GitHub pages, the user typically needs switch to the 'Raw' view.
The function `gitDataUrl()` allows to conventiently switch any url (on GitHub) to the format from 'Raw view', suitable for directly reading the data using _read.delim()_ , _read.table()_ or _read.csv()_ etc ...).
...
## Data Vectorization   {#Vectorization}

In many scientific domains raw data often come as coordinates in form of pairs of start/end points. 
While correlation measures linear association, vector-based metricsÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âdistance and angleÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âcapture both magnitude and direction of change.
The  `vectorDistAngle()` function helps by transforming paired coordinates into polar form (distance and angle).


```{r vectorDistAngle1, echo=TRUE}
x <- matrix(c(0, 3, 0.5, 4, 1, 1), ncol=2, byrow=TRUE)
y <- matrix(c(0, 0, 1, 3, 0.5, 5), ncol=2, byrow=TRUE)
(vectDist <- vectorDistAngle(x, y))
```
Below, a graphical representation of the the example-vectors (which were initially defined by their start- and end-points).
```{r vectorDistAngle2, echo=TRUE}
...
At some point it may rather be better to eliminate a few samples or branches of an experiment to avoid too invasive intervention. This shows that quality control can be tightly linked to decisions about data-normalization.                      {#Normalization}
...
Frequent _NA_-values may represent another potential issue. Unfortunately, there is no general optimal advice for handeling NA-values.
...
High number of NAs in specific columns may point to technical problems or very different samples.
If the second case doesn't seem to apply, one might consider eliminiating such columns of low-quality data. 

If the lines of your data represent different features quantified (eg proteins, or transcripts), then lines almost entirely composed of NA-values represent features
that may not be well exploited anyway. Therefore many times one tries to filter away lines of 'bad' data (ie too high content of NAs).
...
All decisions of which approach to choose should be data-driven.
...
Here it is also possible to custom select only a few pairwise comparisons 
(when an elevated number of groups will trigger a rather high number of pairwise comparisons) :

```{r presenceFilt2, echo=TRUE}
...
### The Function normalizeThis()      {#NormalizeThis}
...
### Normalize By Rows        {#RowNormalize}
...
In some cases/projects we may want to formulate a much more 'engaged' hypothesis : We consider the content of all columns strictly as the same.
For example this may be the case when comparing a dataset consisting of technical replicates only (the data may have many 'holes', ie may have sparse character).
...
Under certain circumstances using the trimmed mean may be more stable towards outlyer values.
The base function  `mean()` allows to preform symmetric trimming, thus with more trimming the result will start converging to the _mean_.
The function `trimmedMean()` (from this package) gives more flexible options for assinging different upper- and lower fractions of the initial data to be trimmed.
When outlyer values always appear at the high (or low) end of data, such proceeding may be useful.
However, the user is encouraged to use this with caution since there is also a risk of introducing bias.


```{r trimmedMean1, echo=TRUE}
x <- c(17:11,27:28)
mean(x)
...
###  Blockwise Normalization Of 2 or 3 Data-Sets   {#blockNormalize}

In some instances you may want to make data-sets globally comparable without completely "normamlizing" all columns, in order to preseve some of the inherent structure of each of the data-sets.
Such procedures typically consiste in rescaling each data-set to common boundaries or to have the same overall median (without median-normalizing each column).

Instead of normalizing all columns of joined data-sets the alternative of blockwise procedures treat each initial data-set as a given block without addressing each column independently.
The function `blockNormalize()` performs block quantile normalization on 2 or 3 datasets (vectors, matrices or data.frames) to align their overall distributions while preserving the internal rank structure within each block.
This function was designed to make 2 or 3 data-sets globally comparable without intervening too much to the internal structures.

As result, all output blocks (up to 2 blocks) will have identical distributions (the mean of their sorted values)
The relative ordering (ie ranks) within each original block is preserved.
This function resembles somehow to the function normalizeBetweenArrays() from package limma, which does quantile normalization - but not for arbitrary blocks (using entire arrays only).


```{r blockNormalize1, echo=TRUE}
...
The primary us of this function will likely consist in normalizing matrixes or data.frames
```{r blockNormalize2, echo=TRUE}
...
You can see that relative variability inside each dataset will show up differently depending on the normalization procedure chosen.

Under some circumstances you may want to define different now ranges for each data-set.
This might be of interest if the data-sets follow somehow different distributions, in particuar in respect to (rare) extreme values.
In some cases it my be of interest to not provide absolute boundaries but given quantiles for lower and/or upper boundaries. 

In some cases it my be of interest not defining absolute boundaries but rather use quantiles for lower and/or upper boundaries. 

the overal distributions look like this:
```{r blockNormalize4, echo=TRUE}
...
In some cases (rare) extreme values may have strong impact on the outcome of re-scaling.
In such cases you can rather define two quantiles of the data to get normalized to predefined values, and the remaining data will get adjusted in a linear fashion.
This way it is poissible to align the major part of 2 datasets while extreme values live their 'own lives' (if you choose to conserve their extreme character).
Otherwise the  _method="quantile.block"_ will help bringing extreme values closer to the main bulk of data.
...
If you would like to run all possible pairwise comparisons you can set argument _useComparison_ to 'all'.

```{r moderTestXgrp2, echo=TRUE}
test8a <- moderTestXgrp(t8, grp, useComparison="all")
head(test8a$p.value, n=3) 
```

Besides, there are multiple ways to select which pairwise comparison(s) you would like to perform.

```{r moderTestXgrp3, echo=TRUE}
...
In some cases one might like a more explicite name of pairwise combined groups.
Using the argument _sep_ a specific separator may be given or searched from groups like _'combine2'_ .

```{r moderTestXgrp4, echo=TRUE}
...
The function `replicateStructure()` aims to provide help by checking all column of a matrix of meta-data with the aim of identifying the replicate-status.
...
There are multiple ways of specifying pairwise comparions (eg 'A-B', or c('A','B'), or using indexes), the function `convPairwiseSetup()`  allows converting between different types 
and returns a list with multiple formats for addressing specific quesions. 
If the separator for concantenating is known this may be helpful, but not obligatory. In many cases the function will manage to identify independently which separator has been used.


```{r convPairwiseSetup1, echo=TRUE}
...
In many cases the user doesn't choose a specific separator to be used for concatenating group-names.  
The function `convPairwiseSetup()` allows checking a panel of separators to choose a 'unique' one which does not occur in any of the group-names. 

```{r suggestSeparator1, echo=TRUE}
getPWseparator(grp=LETTERS[1:3])
getPWseparator(grp=c("B-C","D","E")) 
getPWseparator(grp=c("B-C","D","E"), includeGrp=TRUE) 
```

Of course you can also check your preferred separators, the first found compatible (ie unique) will be reported.
```{r suggestSeparator2, echo=TRUE}
potSep <- c("-", "D", " -compared to- ")
getPWseparator(grp=c("B-C","D","E"), includeGrp=TRUE, potSep=potSep)
```


By using the argument _includeGrp_ the output will be a list including the input instead of simply the suggested separator.
```{r suggestSeparator3, echo=TRUE}
getPWseparator(grp=c("B-C","D","E"), includeGrp=TRUE) 
```
...
The same function may be used to identify which separator has been used, by checking a panel of classical candidates for separators
```{r identifySeparator1, echo=TRUE}
## Identify separator used
getPWseparator(compNames=c("B-C","B-D","C-D")) 
getPWseparator(compNames=c("B-C","B-D","C-D"), includeGrp=TRUE)
```

Besides, the function  `findHeadAndTail()` allows (unbiased) identifying separators without having to define candidates to check,
as it proceeds by removing names of groups from beginning and end of concatenated terms. 
However, there is a slight risk that some special characters may not allow finding the proper solution.

```{r identifySeparator2, echo=TRUE}
## Identify separator used
...
Please note that this function shares some overlap with _adjBy2ptReg()_ (which has less options and will get deprecated in the future)
...
The author would like to acknowledge the support by the [IGBMC](https://www.igbmc.fr) (CNRS UMR 7104, Inserm U 1258, UdS), [CNRS](http://www.cnrs.fr/en), 
[Universit&eacute; de Strasbourg (UdS)](https://www.unistra.fr/fr) and [Inserm](https://www.inserm.fr).
