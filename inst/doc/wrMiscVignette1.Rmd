...
Similarly, we can also calculate the sd or CV for each line while splitting into groups of replicates (`rowGrpSds` and `rowGrpCV`): 
...
The function `filterList()` aims facilitating the filtering of all elements of lists or S3-objects. List-elements with inapropriate number of lines will be ignored.
...
the function `combineByEitherFactor()` will help. So basically, this is an empric segmentation-approach for two categorical variables.
...
Using `closeMatchMatrix()` the result obtained above, can be presented in a more convient format for the human eye.
...
The function `readCsvBatch()` was designed for reading multiple csv files of exactely the same layout and to join their content.
...
The function `readVarColumns()` (from this package) provides help to read such data, if the content (and separtors) of the last columns are missing.
...
When running multiple pairwise tests (using *moderTestXgrp()*) the results will be in adcent columns and the group-names reflected in the column-names.
In the case measurements from multiple levels of a given variavle are compared it is useful to extract the numeric part, the function `numPairDeColNames()` provides support to do so. 
When extracting just the numeric part, unit names will get lost, though. Note, if units used are not constant (eg seconds and miliseconds mixed) the extracted numeric values do not relflect the real quantitative context any more.
