...
#' @return array (or list if \code{returnArray=FALSE}) of all numeric data read (numerical columns only) from individual files
...
readCsvBatch <- function(fileNames=NULL, path=".", fileFormat="Eur", checkFormat=TRUE, returnArray=TRUE,
  ...
  if(is.null(fileNames)) {   # read btach of files
    if(length(path) <1) path <- "." else { chPath <- file.exists(path)
      if(!chPath) {path <- "."
      message(fxNa," Cannot find path '",path,"' !  ... Setting to default='.'")}}
  ...
  if(length(fileNames) >0) {
    ...
    tmp1 <- if(!identical(fileFormat,"Eur")) try(utils::read.csv(fileNames[1], header=HeaderInData, stringsAsFactors=FALSE)) else NULL
    tmp2 <- if(!identical(fileFormat,"US")) try(utils::read.csv2(fileNames[1], header=HeaderInData, stringsAsFactors=FALSE)) else NULL
    cheF <- "try-error" %in% c(class(tmp1), class(tmp2)) 
    if(any(cheF)) { fileNames <- NULL
