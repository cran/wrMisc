...
#' 
#' @param n (integer)  number of observations. If \code{length(n) > 1}, the length is taken to be the number required.
...
#' @export
rnormW <- function(n, mean=0,sd=1,seed=NULL, callFrom=NULL, silent=FALSE) {
  ...
  if(length(n) <1 | !is.numeric(n)) { out <- NULL; if(!silent) message(fxNa,"invalid 'n', returning NULL")
    ...
    if(!is.numeric(sd)) mean <- try(as.numeric(sd))
    if(length(sd) != 1 | "try-error" %in% class(sd)) stop(msg[1],"sd",msg[2])
    ## main
    if(n==1) out <- mean 
    if(n==2) out <- mean + c(-1,1)*sd/sqrt(2) 
    if(n >2) {
      if(length(seed) ==1) try(set.seed(seed), silent=TRUE)
      out <- signif(stats::rnorm(n=n, mean=mean, sd=sd), 6)
