...
#'
#'  \code{diff()}-like function to return difference in ppm between psubsequent values. 
...
diffPPM <- function(dat,toPrev=FALSE,silent=FALSE,callFrom=NULL){
  ...
  if(!is.numeric(dat)) dat <- try(as.numeric(if(is.factor(dat)) as.character(dat) else dat))
  if(class(dat) == "try-error") {if(!silent) message(fxNa,"Can't convert 'dat' to numeric !"); return(NULL)
  } else {
