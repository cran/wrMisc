...
#'
#' This function calculates CVs of replicates from list of 2 or 3-dim arrays (where 2nd dim is replicates, 3rd dim may be channel). 
...
#' @return This function returns a matrix or array (1st dim is intraplate-position, 2nd .. plate-group/type, 3rd .. channels)
...
#' replPlateCV(arrL1)
#' @export
replPlateCV <- function(lst, silent=FALSE, debug=FALSE, callFrom=NULL) {
  ...
  if(is.matrix(chDim)) {if(any(apply(chDim,1,function(x) length(unique(x))>1))) stop(fxNa,msg)} else stop(fxNa,msg)
