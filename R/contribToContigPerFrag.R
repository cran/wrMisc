...
#' @return This function returns a matrix of 3 columns for each node identified in rownames of \code{joinMatr} for length of longest tree-branches where given edge participates (column \code{sumLen}), the (total) number of edges therein (col \code{n.frag}) and a relative value (\code{len.rat})
...
#' @export
contribToContigPerFrag <- function(joinMat, fullLength=NULL, nDig=3, silent=FALSE, callFrom=NULL, debug=FALSE){
  fxNa <- .composeCallName(callFrom, newNa="contribToContigPerFrag")
  ...
  if(length(dim(joinMat)) <2) stop(fxNa,msg) else if(!"sumLen" %in% colnames(joinMat)) stop(fxNa,"Need column 'sumLen' in ",argN)
