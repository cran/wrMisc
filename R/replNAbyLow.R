...
#' @return This function retuirns a numeric matrix (same dim as 'x') with \code{NA} replaced
#' @seealso \code{\link{naOmit}}, \code{\link[stats]{na.fail}}
#' @examples
#' dat <- matrix(round(rnorm(30),2),ncol=6)
#' grD <- gl(2,3)
#' dat[sort(sample(1:30,9,repl=FALSE))] <- NA
#' dat; replNAbyLow(dat,gr=grD)
#' @export
replNAbyLow <- function(x, grp, quant=0.8, signific=3, unif=TRUE, absOnly=FALSE, seed=NULL, silent=FALSE, debug=FALSE, callFrom=NULL) {
  fxNa <- .composeCallName(callFrom, newNa="replNAbyLow")
  if(length(dim(x)) <2) stop(fxNa, "Expecting matrix or data.frameas 'x'")
  if(is.data.frame(x)) x <- as.matrix(x)
  if(length(grp) != ncol(x)) stop(fxNa, "'grp' does NOT match number of columns in 'x'")
  nNA <- sum(is.na(x))
  if(nNA >0) {
    ...
    if(!is.null(seed)) {if(is.numeric(seed)) set.seed(round(seed[1])) else if(!silent) message(fxNa, "Ignoring invalid 'seed'")}
    ...
    if(absOnly && any(raVa <0, na.rm=TRUE) && !silent) {message(fxNa, "Force ",sum(raVa <0)," to positive"); raVa <- abs(raVa)}
    tmp <- unique(raVa)
