...
#' @return This function returns a numeric vector without extreme values or index-position of extreme values 
#' @seealso \code{\link{firstOfRepLines}},  \code{\link{get1stOfRepeatedByCol}} for treatment of matrix
#' @examples
#' x <- c(rnorm(30),-6,20)
#' exclExtrValues(x)
#' @export  
exclExtrValues <- function(dat, result="val", CVlim=NULL, maxExcl=1, showNA=FALSE, goodValues=TRUE, silent=FALSE, debug=FALSE, callFrom=NULL) {
  ...
  if(length(extrVal) == length(dat) && !silent) message(fxNa,"All values are equidistant, not possible to distinguish one/some as 'extreme'")
  if(length(extrVal) > maxExcl && !silent) message(fxNa,"More extreme values are equidistant than available for display with 'maxExcl' = ",maxExcl)
  if(!is.null(CVlim)) if(!is.finite(as.numeric(CVlim))) {
    warning(fxNa,"'CVlim' should be numeric of length 1, ignoring current value of 'CVlim'"); CVlim <- NULL}
  if(!is.null(CVlim)) {
