...
#' @return This function returns a combined matrix (elements not found in 'annotTab' are displayed as NA)
...
fuseAnnotMatr <- function(iniTab, annotTab, refIniT="Uniprot", refAnnotT="combName",
  ...
  if(length(uniqGe) == nrow(iniTab)) {
    ...
    supAnn <- matrix(NA_real_, nrow(iniTab), length(useCol), dimnames=list(iniTab[,"Uniprot"], colnames(annotTab)[useCol]))  # (empty) for cbind
