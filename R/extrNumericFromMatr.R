...
#'
#' This function extracts numeric part of matrix or data.frame, removing remaining non-numeric elements if \code{trimToData} is set to \code{TRUE}.
...
#' @return This function reurns a matrix of numeric data
#' @examples
...
#' mat; extrNumericFromMatr(mat)
#' @export
extrNumericFromMatr <- function(dat, trimToData=TRUE, silent=FALSE, debug=FALSE, callFrom=NULL) {
  ...
  out <- matrix(NA_real_, nrow=nrow(dat), ncol=ncol(dat))
