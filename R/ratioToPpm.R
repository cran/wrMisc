...
#' @return This function returns a numeric vector of ppm values
...
#' @export
ratioToPpm <- function(x, y=NULL, nSign=NULL, silent=FALSE, debug=FALSE, callFrom=NULL){
  ...
  if(length(y) == length(x) || length(y) ==1) {                      # get ration of  x / Y
   if(any(y==0, na.rm=TRUE) && !silent) message(fxNa,"Division by 0 !")
    x <- x/y }
  if(any(x <0)) {if(!silent) message(fxNa,"Ngative values not possible"); x <- abs(x)}
  whNeg <- which(x <1)
  x[whNeg] <- 1/x[whNeg]
  x <- 1e6*(1-x)
