...
#' @param testSynerg (logical) decide if factor-interactions (eg synergy) should be tested in model, otherwise additive factors are supposed
...
#' @export
test2factLimma <- function(datMatr, fac1, fac2, testSynerg=FALSE, testOrientation="=", addResults=c("lfdr","FDR","Mval","means"), addGenes=NULL, silent=FALSE, callFrom=NULL, debug=FALSE){
  fxNa <- .composeCallName(callFrom, newNa="test2factLimma")
  if(isTRUE(debug)) silent <- FALSE else { debug <- FALSE
    if(!isTRUE(silent)) silent <- FALSE }
  doTest <- TRUE
  if(!requireNamespace("limma", quietly=TRUE)) { doTest <- FALSE
    message(fxNa,"NOTE : You need to install first the package 'limma' from Bioconductor")}
  if(doTest) {    
  ...
  if(doTest && requireNamespace("limma", quietly=TRUE)) {    
  ...
  if(doTest && requireNamespace("limma", quietly=TRUE)) {    
    ...
    if(length(addResults) >0) if("lfdr" %in% tolower(addResults) && !requireNamespace("fdrtool", quietly=TRUE)) {
      ...
      if("FDR" %in% toupper(addResults)) out$FDR <- if(length(dim(out$p.value)) >1) {
        ...
        if("lfdr" %in% tolower(addResults)) {
          if(requireNamespace("fdrtool", quietly=TRUE)) {
            ...
            if(inherits(out$lfdr, "try-error")) { if(!silent) message(fxNa,"PROBLEM with calulating lfdr ! ")
