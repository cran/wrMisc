...
#' @export
mergeVectors <- function(..., namePrefix="x.", NAto0=FALSE, callFrom=NULL, silent=FALSE) {
  ...
  if(length(inpL) >0) {
    if(NAto0) inpL <- lapply(inpL, function(x) {chNa <- is.na(x); if(any(chNa)) x[which(chNa)] <- 0; x})
