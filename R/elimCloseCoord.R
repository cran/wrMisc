...
#'
#' This function reduces number of rows in 'dat' by eliminating lines where x & y coordinates (columns of matrix '\code{dat}' defined by '\code{useCol}') are identical (overlay points) or very close.
...
#' @return This function returns the resultant matrix/data.frame
...
elimCloseCoord <- function(dat, useCol=1:2, elimIdentOnly=FALSE, refine=2, nDig=3, callFrom=NULL, silent=FALSE){
  ...
  if(!elimIdentOnly) {
    if(nDig <3 && !silent) message(fxNa,"The stringency chosen (of ",nDig," significant digits) is very high, you may consider using 'nDig' values higher than 2 !")
