  ...
  if(chPa) {
    ...
    if(inherits(txt, "try-error") | length(txt) <2) { Sys.sleep(30)
      ...
      if(inherits(txt, "try-error") | length(txt) <2) { txt <- try(readLines(countUrl, warn=FALSE, n=9000), silent=TRUE)
        if(!inherits(txt, "try-error")) message(fxNa," Failed to read entire data-set, but succeeded to read first 9000 lines. Your package(s) of interest may not be included in this list")
      }
    }
    ## start exploiting
    ...
    } else if(!silent) message(fxNa,"Unable to read data (maybe the server is not responding ?)")
