...
#' @export
cbindNR <- function(..., convertDFtoMatr=TRUE, sortOutput=TRUE, summarizeAs="sum", silent=FALSE, debug=FALSE, callFrom=NULL){
  ...
  if(length(inpL) <1) stop(fxNa," It seems the input doesn't contain any (valid) matrixes")
  matrDim <- sapply(inpL,dim)
  if(length(unique(matrDim[1,])) >1) stop(fxNa," Some matrixes don't have the same number of rows (found ",paste(matrDim[1,],collapse=" "),")")
