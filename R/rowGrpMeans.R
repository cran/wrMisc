...
#' @return This function returns a matrix with mean values
#' @seealso \code{\link{rowSds}}, \code{\link[base]{colSums}} which also shows \code{colMeans} 
#' @examples
#' set.seed(2016); dat1 <- matrix(c(runif(200) +rep(1:10,20)), ncol=10)
#' head(rowGrpMeans(dat1, gr=gl(4, 3, labels=LETTERS[1:4])[2:11]))
#' @export
rowGrpMeans <- function(x, grp, na.rm=TRUE, silent=FALSE, debug=FALSE, callFrom=NULL){
