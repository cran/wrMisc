...
presenceFilt <- function(dat,grp,maxGrpMiss=1,ratMaxNA=0.8,minVal=NULL,silent=FALSE,callFrom=NULL){           
  ...
  nNa <- matrix(unlist(by(t(dat),grp,function(x) colSums(is.na(x)))),nrow=nrow(dat))  # no of NAs per group & line
  ...
  colnames(out) <- outNa <- paste(outNa[,1],outNa[,2],sep="-")
  out }
...
#' @export
.filterMinAv <- function(dat,thr,silent=FALSE,callFrom=NULL){
  ## filter matrix to lines with average >= 'thr' (ie remove lines with low values)
  fxNa <- .composeCallName(callFrom,newNa=".filterMinAv")
  noSamCounts <- colSums(dat,na.rm=TRUE)
  datNor <- dat*mean(noSamCounts,na.rm=TRUE)/matrix(rep(noSamCounts,each=nrow(dat)),nrow=nrow(dat))
  datFi <- dat[which(rowMeans(datNor,na.rm=TRUE) >= thr),]
  if(!silent) message(fxNa," reducing from ",nrow(dat),"  to ",nrow(datFi)," lines")
  datFi }
