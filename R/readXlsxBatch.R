...
#' NOTE : (starting from version wrMisc-1.5.5) requires package \href{https://CRAN.R-project.org/package=readxl}{readxl} being installed ! 
    ...
    for(i in 1:length(useFi)) {
      ...
      if(length(tmp) >0) {
        ...
        if(checkFormat) {
          ...
          outL[[i]] <- .inspectHeader(outL[[i]], headNames=columns,silent=silent | i !=1, callFrom=fxNa)
          outL[[i]] <- .removeEmptyCol(outL[[i]], fromBackOnly=FALSE,silent=silent | i !=1, callFrom=fxNa)
        }
        ...
        if(i ==1) {                         # (re)define new format based on 1st file after format-checking (ie remove empty cols, extract col of well-names,...)
          outDim <- dim(outL[[i]])       
          out <- if(returnArray) array(NA, dim=c(outDim[1],length(fileNames),outDim[2]), dimnames=list(rownames(outL[[i]]), basename(fileNames),colnames(outL[[i]]))) else NULL }
