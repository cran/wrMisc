...
#' @param callFrom (character) allows easier tracking of messages produced#' @return This function returns a matrix of 6 columns: input (beg and end), beg.n, beg.rat, end.n, end.rat
...
#' simpleFragFig(frag1)
#' @export
countSameStartEnd <- function(frag, minFreq=2, nDig=4, silent=FALSE, debug=FALSE, callFrom=NULL){
  fxNa <- .composeCallName(callFrom, newNa="countSameStartEnd")
  if(!isTRUE(silent)) silent <- FALSE
  if(isTRUE(debug)) silent <- FALSE else debug <- FALSE

  if(!is.matrix(frag)) frag <- as.matrix(frag)
  if(is.null(rownames(frag))) rownames(frag) <- paste0("li",1:nrow(frag)) else {
    frag <- frag[sort.list(rownames(frag)),,drop=FALSE]}
