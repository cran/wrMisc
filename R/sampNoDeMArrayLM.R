...
sampNoDeMArrayLM <- function(MArrayObj, useComp, groupSep="-", lstMeans="means", lstP=c("BH","FDR","p.value"), silent=FALSE, debug=FALSE, callFrom=NULL) {
  ...
  if(length(colnames(MArrayObj[[lstMeans]])) <1) stop(fxNa,"Problem with 'MArrayObj$lstMeans' (does not contain matrix of means)")
  if(ncol(MArrayObj[[lstMeans]])==2) {             # only 2 mean-values, no other choice, don't need to try matching anything
    ...
    if(length(colnames(MArrayObj[[lstP]])) <1)  stop(fxNa,"Problem with 'MArrayObj' (can't find names of comparisons in '",lstP,"')")
    ...
    if(length(useComp) <1) stop(fxNa,"Argument 'useComp' is empty or can't locate in comparison-names")
    ## main
