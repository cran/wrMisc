...
#'
#' \code{searchDataPairs} searches matrix for columns of similar data, ie 'duplicate' values in separate columns or very similar columns if 'realDupsOnly'=FALSE.
#' Initial distance measures will be normalized either to diagonale (normRange=TRUE) of 'window' or to the real max distance observed (equal or less than diagonale).
...
#' @param normRange (logical) normize each columns separately if TRUE
#' @param altNa (character, default NULL) vector with alternative names (for display)
...
#' @return  data.frame with names for sample-pair, percent of identical values (100 for complete identical pair) and rel (Euclidean) distance (ie max dist observed =1.0)
#' @seealso \code{\link[base]{duplicated}}, \code{\link[stats]{dist}}
#' @examples
