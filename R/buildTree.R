...
#' @return list with $paths (branches as matrix with columns 'sumLen' & 'n'), $usedNodes (character vector of all names used to build tree) and $tree (object from data.tree)
...
.growTree <- function(tm,setX,addToObj=NULL) {
  ...
  #addToObj <- if(tm$iter==1 & length(addToObj) >0) addToObj else paste("b",tm$it[j],"_",j,sep="")
  newNodeNa <- paste("b",0,"_",tm$disDat[tm$start,1],sep="")
  ...
  while(any(tm$lo)) {        # need to grow further ..
    ...
    assign(paste("b",tm$it[j],"_",j,sep=""), get(addToObj)$AddChild(rownames(tm$disDat)[j],len=tm$disDat[j,3])) 
    ...
    if(any(tm0)) {z <- which(tm0);                        
      ...
      tm$preN[z] <- paste("b",tm$it[j],"_",j,sep="")          # report (prev)name of node
