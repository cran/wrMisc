...
#'
#' This function does rescaling: linear transform simple vector 'inDat' that (mean of) elements of names cited in 'refLst' will end up as values 'regrTo'. 
...
#' @param inDat (matrix or data.frame) data to rescaled 
...
#' @return This function returns a normalized matrix
...
#' plot(reg1, dat1) 
#' @export
regrBy1or2point <- function(inDat, refLst, regrTo=c(1,0.5), silent=FALSE, debug=FALSE, callFrom=NULL){
  ...
  if(length(names(inDat)) <0) {
    ...
    if(!silent) message(fxNa,"No (valid) names found in 'inDat', making default numbering")}
