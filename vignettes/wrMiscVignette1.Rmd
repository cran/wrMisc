...
If necessary, a counter can be added to non-unique entries, thus no individual values get eliminated and the length and order of the resultant object maintains the same.
...
As you can see from this example above, looking for the most frequent exact value is very frequently not a perfect choice for continous data. 
In this example the method _'allModes'_ gave partially usable results (dashed grey lines), due to the rounding to 3 digits. 
of course, this approach may give ties (as shown above).
